...
def fun(n, Fmj_set, mj, A_set, N, NN, m):
    ...
    for indj, j in enumerate(N):
        ...
        # Relative RMSE (MC):
        RRMSE[indj, 0] = RMSEcalculate(ESTi3, exbel)/exbel

        # Relative RMSE (RQMC with random shifted Sobol):
        RRMSE[indj, 1] = RMSEcalculate(ESTqSi3, exbel)/exbel
    ...
    print('==========================================================================')
    A, B = curve_fit(f, np.log10(N), np.log10(RRMSE[:, 0]))[0]  # your data x, y to fit
    print('The slope for MC is:', A)
    A, B = curve_fit(f, np.log10(N), np.log10(RRMSE[:, 1]))[0]  # your data x, y to fit
    print('The slope for RQMC is:', A)

    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.loglog(N, RRMSE[:, 0], '-*', markersize=12, color='k',
               label=r'$P^{*}_{\mathcal{S}(A)} // P^{*}_{\mathcal{S}(\Theta)}$\textit{ MC}')
    plt.loglog(N, RRMSE[:, 1], '-gD', markersize=8, color='k',
               label=r'$P^{*}_{\mathcal{S}(A)} // P^{*}_{\mathcal{S}(\Theta)}$\textit{ RQMC}')

    plt.xlabel(r'\huge$N$')
    plt.ylabel(r'\huge\textit{Reltive RMSE}')
    ...
    print(str(
        n) + 'masses.pdf containing the plots in loglog-scale corresponding to MC and RQMC (Relative RMSE vs N) was created!')
    plt.clf()
    # plt.show()
