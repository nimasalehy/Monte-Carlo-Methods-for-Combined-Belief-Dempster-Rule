...
def fun(n, Fmj_set, mj, A_set, N, NN, m, usi1, tusi1, ui2i3, tui2i3):
    ...
    for indj, j in enumerate(N):
        ...
        # RMSEs
        RMSE[indj, 0] = RMSEcalculate(ESTs, exbel)
        RMSE[indj, 1] = RMSEcalculate(ESTi1, exbel)
        RMSE[indj, 2] = RMSEcalculate(ESTi2, exbel)
        RMSE[indj, 3] = RMSEcalculate(ESTi2new, exbel)

        RMSE[indj, 4] = RMSEcalculate(ESTi3, exbel)
        RMSE[indj, 5] = RMSEcalculate(ESTi3new, exbel)
        RMSE[indj, 6] = RMSEcalculate(ESTi32, exbel)
        RMSE[indj, 7] = RMSEcalculate(ESTi3new2, exbel)
...
N = [20000, 40000]
NN = N[len(N) - 1]
m = 10  # Number of samples, each with size N
